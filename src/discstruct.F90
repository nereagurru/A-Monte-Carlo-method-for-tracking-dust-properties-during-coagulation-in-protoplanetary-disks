...
module discstruct
    ...
    real function Temp(x, time)
         ...
         if ( (time >= period .and. time <=  period+dt) .or. &
              ...
              (time >= 6.*period .and. time <= 6.*period+dt) .or. &
              (time >= 7.*period .and. time <= 7.*period+dt) .or. &
              (time >= 8.*period .and. time <= 8.*period+dt) .or. &
              (time >= 9.*period .and. time <= 9.*period+dt) .or. &
              (time >= 10.*period .and. time <= 10.*period+dt) .or. &
              (time >= 11.*period .and. time <= 11.*period+dt) ) then
            factor = (100.)**(0.25)
         end if


         Temp = factor * temperature*(x/AU)**(-q)
