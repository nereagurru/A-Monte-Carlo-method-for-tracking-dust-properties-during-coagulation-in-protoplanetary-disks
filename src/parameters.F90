...
module parameters
    ...
    public :: read_parameters, Ntot, ncell, nr, nz, fout, dtime, tend, smallr, restart, restime, minrad0, maxrad0, &
              r0, dmmax, eta, dtg, vfrag, db_data, tolerance, &
              datadir, path, sigmag0, temperature, alpha_t, nbins
#ifdef MULTI_COMPONENT
   public :: matdensw, matdenssi
#else
   public :: con1, con2, matdens
#endif
#ifdef EROSION
...
#ifdef MULTI_COMPONENT
    real, protected                     :: matdenssi,  matdensw ! different densities for different materials
#else
    real, protected                     :: matdens  ! material density
#endif
    real                                :: dmmax    ! MC-acceleration parameter
    real                                :: tolerance    ! MC-acceleration parameter
      ...
      do while (ios == 0)
            ...
            if (ios == 0) then
               ...
               case('material_density_[g/cm3]')
                     read(buffer, *, iostat=ios) matdensw
                     print *, 'Read material density: ', matdensw
               case('material_density_2[g/cm3]')
                  read(buffer, *, iostat=ios) matdenssi
                  print *, 'Read material density: ', matdenssi
#else
               case('material_density_[g/cm3]')
                ...
                case('merging_parameter_X')
                     read(buffer, *, iostat=ios) tolerance
                     print *, 'Read merging parameter: ', tolerance
...
#ifdef KERNEL_TEST
               case('repeat_test')
                  read(buffer, *,iostat=ios) repeat
                  print *, 'repeat test how many times?', repeat
#endif
