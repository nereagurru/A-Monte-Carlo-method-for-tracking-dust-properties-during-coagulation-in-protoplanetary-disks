...
module hdf5output
  ...
  contains
  subroutine hdf5_file_write (this, swrm, time, outnr, resdt)
    ...
    call h5open_f(error)
    resdtout = resdt/year
    call hdf5_file_create(this, filepath, outnr , resdtout)     ! creates file
...
! subroutine to create dataset for writing output time
  ...
  subroutine create_swarm_type(this,tempswrm)
    ...
    call h5tinsert_f(this%memtype, "mass of swarm [g]", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%mswarm)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
    call h5tinsert_f(this%memtype, "npar", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%npar)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
    ...
    call h5tinsert_f(this%memtype, "Vertical velocity v_z [cm/s]", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%velz)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
#else
   call h5tinsert_f(this%memtype, "fw", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%fw)), &
                     H5T_NATIVE_DOUBLE, this%error)                                                                              
   call h5tinsert_f(this%memtype, "rhoi", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%rhoi)), &
                     H5T_NATIVE_DOUBLE, this%error)
  ...
  subroutine hdf5_file_create(this, filename, nout, resdtout)
    implicit none
    type(hdf5_file_t), intent(out) :: this
    ...
    call hdf5_write_att(this%id, 'material_density_[g/cm3]', matdensw)
    call hdf5_write_att(this%id, 'material_density_2[g/cm3]', matdenssi)
#else
  ...
  subroutine hdf5_file_read(Ntot, swrm, nout, time, resdt)
    implicit none
  ...
  subroutine hdf5_read_att(nout, resdt)
    implicit none
    type(hdf5_file_t) :: this
    type(C_PTR) :: f_ptr
    real, target :: outnr, resdt
