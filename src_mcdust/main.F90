...
program main
   ...
   do while (time < tend)
      ...
      write(*,*) 'condensation time ', con_timescale/year
      ! This is to check that we conserve water in phase change, and that the only variation comes from collisions
      write(*,*) 'Total water content before phase change.... ', vapour_mass + mswarm*sum(swrm(:)%w)
      ...
      write(*,*) 'Total water content after phase change.... ', vapour_mass + mswarm*sum(swrm(:)%w)
