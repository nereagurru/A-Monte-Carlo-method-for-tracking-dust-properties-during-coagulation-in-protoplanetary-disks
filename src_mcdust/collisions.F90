...
module collisions
...
#else
   use parameters, only: matdens, con1, con2 
#endif
#ifdef EROSION
   ...
   subroutine mc_collisions(nr, ni, bin, swrm, dtime, realtime, kk, first_idx)
      ...
      do while (local_time < dtime)
         ...
         !write(*,*) 'dt vs. dtime ', dt/year, dtime/year
   ...
   subroutine col_rates(nl, swarms, relvels, colrates, accelncol, vol, deltar, deltaz)
      ...
      colrates(nl,:) = swarms(:)%npar * relvels(nl,:) * pi**third * (0.75)**(2. * third) * &
                      ((swarms(nl)%mass/swarms(nl)%rhoi)**third + (swarms(:)%mass/swarms(:)%rhoi)**third)**2./vol
#else
...
#ifdef MULTI_COMPONENT
      swarms(nri)%npar = mswarm / swarms(nri)%mass * (1. + swarms(nri)%w)
#else
...
#ifdef MULTI_COMPONENT
      swarms(nri)%fw = (swarms(nri)%mass*swarms(nri)%fw + accelncol(nri, nrk) * swarms(nrk)%mass * swarms(nrk)%fw) / &
