...
module advection
   ...
   use parameters, only: matdens, con2
#endif
   use parameters, only: smallr
   implicit none
   ...
   real function stokesnr(particle, realtime)
      ...
      rad = (0.75 / pi / particle%rhoi * particle%mass) **third
#else
      rad = con2 * particle%mass**third
#endif
      if (rad > 2.25 * lmfp) then ! Stokes regime
#ifdef MULTI_COMPONENT
         stokesnr = sqrt(2.*pi) * rad**2. * particle%rhoi * omegaK(particle%rdis) * AH2 / (9. * css * mH2)
#else
         stokesnr = sqrt(2.*pi) * rad**2. * matdens * omegaK(particle%rdis) * AH2 / (9. * css * mH2)
#endif
      else                 ! Epstein regime
#ifdef MULTI_COMPONENT
         stokesnr = rad * particle%rhoi / (sqrt(8./pi) * css * gasdens) * omegaK(particle%rdis)
#else
...
#ifdef MULTI_COMPONENT
         write(*,*)'dens', gasdens, 'omegaK', omegaK(particle%rdis),'size', rad,  'cs', css, 'fw', particle%fw
         write(*,*)'dens', particle%rhoi
#endif
